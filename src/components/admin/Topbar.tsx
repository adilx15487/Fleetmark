        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div ref={dropdownRef} className="relative">
            ...
            >
              ...
              <span className="hidden sm:block text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{user?.name.split(' ')[0] || 'Admin'}</span>
            ...
            {profileOpen && (
              <div className="absolute right-0 mt-2 w-48 rounded-xl shadow-xl overflow-hidden" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border-subtle)' }}>
                <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                  <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{user?.name || 'Admin'}</p>
